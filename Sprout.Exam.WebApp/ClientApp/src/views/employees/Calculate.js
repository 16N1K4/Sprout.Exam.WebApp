...
export class EmployeeCalculate extends Component {
  ...
  handleChange(event) {
      ...
      this.setState({ [event.target.name]: value }, () => console.log(this.state.absentDays));
  }
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async calculateSalary() {
    ...
    const requestOptions = {
      ...
      var roundedIncome = data.toFixed(2);
    this.setState({ loadingCalculate: false, netIncome: roundedIncome });
  }
